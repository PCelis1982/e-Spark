...
export default function LandingPage({ openWalletModal }) {
  ...
  const showLoginButton = import.meta.env.VITE_SHOW_LOGIN === 'true';
  ...
  return (
    <div className="relative min-h-screen w-full font-sans text-white">
      ...
      />
      <div className="relative z-10 bg-gradient-to-b from-[#1A1A1A]/90 to-black/90 min-h-screen w-full">
        <header className="sticky top-0 z-50 flex justify-between items-center p-6 bg-[#1A1A1A]/80 backdrop-blur border-b border-cyan-500">
          <img src="/assets/e-Spark - IFM.png" alt="e-Spark logo" className="h-10" />
          <nav className="flex space-x-6 text-sm">
            {['who', 'partners', 'products', 'contact'].map((id) => (
              <a
                key={id}
                href={`#${id}`}
        ...
        <main className="py-20 px-6 space-y-40 max-w-5xl mx-auto">
          <section className="min-h-screen flex flex-col items-center justify-center text-center px-6">
            <img src="/assets/e-Spark - IFM.png" alt="Main logo" className="w-3/5 mb-6" />
          </section>

          <section id="who" className="text-white bg-black"><WhoWeAre /></section>
          <section id="partners" className="text-white bg-black"><PartnersSection /></section>
          <section id="products" className="text-white bg-black"><ProductSection /></section>

          <section id="contact" className="py-20 px-4 text-white bg-black">
          ...
          {userProfile && (
            <section className="bg-[#1A1A1A] border border-cyan-500 p-6 rounded-lg text-white">
              <h2 className="text-2xl font-bold text-cyan-400 mb-4">User Profile</h2>
              <p><strong>Email:</strong> {userProfile.email}</p>
              <p><strong>Role:</strong> {userProfile.role}</p>
              <p><strong>Wallet:</strong> {userProfile.walletAddress || "Not connected"}</p>
            </section>
          )}
        </main>
          ...
          <div className="fixed inset-0 bg-black/90 flex justify-center items-center z-50">
            <Card className="bg-[#1A1A1A] text-white w-96 p-6 border border-cyan-400">
              <CardContent className="space-y-4">
                ...
                <Button className="w-full bg-blue-600 text-white" onClick={loginWithGoogle}>Continue with Google</Button>
                <Button className="w-full bg-cyan-600 text-black" onClick={openWalletModal}>Connect Wallet</Button>
